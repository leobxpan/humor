...
OUT_FPS = 30
SAVE_MOJO_VERTS = True # save MOJO vertex locations
...
ALL_DATASETS = ['bedroom0122', 'bedroom0123', 'bedroom0210',
               'classroom0219', 'dormitory0218', 'garden0214',
               'kitchen0214', 'lab0220', 'library0219', 'livingroom0129',
               'livingroom0210', 'meetingroom0220', 'middle_hall0113',
               'office0110', 'seminar_room0_0219', 'seminar_room0_0221',
               'seminar_room1_0219', 'seminar_room1_0221', 'storeroom0217']
...
def get_body_model_sequence(smplh_path, gender, num_frames,
    ...
    betas = torch.zeros(num_frames, NUM_BETAS).to(device)
...
def process_seq(data_paths):
    ...
    input_file_path, output_file_path, smplx_path = data_paths
    ...
    try:
        ...
        print('Could not open npz file. Skipping...')
        print(e)
        return
    ...
    # for smplx, joint order is root orient (3), body (63), jaw (3), eyel (3), eyer (3), then hands
    pose_hand = bdata['poses'][:, 75:]      # finger articulation joint rotations
    ...
    while process_inds[0] < num_frames:
        ...
        body = get_body_model_sequence(smplx_path, gender, process_inds[1] - process_inds[0],
        ...
        if SAVE_MOJO_VERTS:
            ...
            cur_mojo_seq = cur_vtx_seq[:,MOJO_VERTS,:]
    ...
    joints_world2aligned_rot = None
    if SAVE_JOINT_FRAME and SAVE_VELOCITIES:
    ...
    if OUT_FPS != fps:
        if OUT_FPS > fps:
            ...
            downsamp_inds = np.linspace(0, num_frames-1, num=new_num_frames, dtype=np.int)
            # print(downsamp_inds)
            ...
            if SAVE_JOINT_FRAME and SAVE_VELOCITIES:
                # already have the joint frames, downsample them accordingly
                joints_world2aligned_rot = joints_world2aligned_rot[downsamp_inds]
    ...
    if SAVE_ALIGN_ROT:
        ...
        if SAVE_JOINT_FRAME and not SAVE_VELOCITIES:
            # need to compute them for just the downsampled steps
            joints_world2aligned_rot = compute_joint_align_mats(joint_seq)
    ...
    np.savez(output_file_path, fps=fps,
                               ...
                               joints_world2aligned_rot=joints_world2aligned_rot)
...
def main(config):
    ...
    all_dataset_dirs = [os.path.join(config.gimo_root, f) for f in sorted(os.listdir(config.gimo_root)) if f[0] != '.']
    ...
    # requested datasets
    dataset_dirs = [os.path.join(config.gimo_root, f) for f in config.datasets]
