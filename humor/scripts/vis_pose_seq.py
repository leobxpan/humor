...
seq = "/scr/bxpan/gimo_processed/bedroom0122/2022-01-21-194925/poses_838_frames_30_fps.npz"
...
neutral_bm = BodyModel(bm_path=neutral_bm_path, num_betas=10, batch_size=seq['pose_body'].shape[0])
...
body_pred = neutral_bm(pose_body=torch.from_numpy(seq['pose_body']).float(), \
                       ...
                       betas=torch.from_numpy(seq['betas'][:seq['pose_body'].shape[0]]).float(), \
                       root_orient=torch.from_numpy(seq['root_orient']).float(), \
                       trans=torch.from_numpy(seq['trans']).float())
