...
from utils.torch import get_device_from_gpu_arg, DataParallelPassthrough, save_state, load_state
...
def train(args_obj, config_file):
    ...
    train_file_path = os.path.join(pkg_root, 'train/train_motionvae.py')
    ...
    # parallelize if desired and put model on device
    device, parallel_train = get_device_from_gpu_arg(args.gpu)
    if parallel_train:
        model = DataParallelPassthrough(model)
        loss_func = DataParallelPassthrough(loss_func)
    model.to(device)
    loss_func.to(device)
    ...
    if args.ckpt is not None:
        ...
        start_epoch, min_val_loss, min_train_loss = load_state(args.ckpt, model, optimizer=load_optim, is_parallel=parallel_train, map_location=device, ignore_keys=model.ignore_keys)
        start_epoch += 1
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            try:
                ...
                if torch.isnan(loss).item(): # and epoch > 0:
                ...
                # print('TOTAL NORM (before): %f' % (total_norm))
                ...
                #     # total_norm = torch.norm(torch.stack([torch.norm(p.grad.detach(), 2.0).to(device) for p in parameters]), 2.0)
                #     # print('TOTAL NORM (after): %f' % (total_norm))
                optimizer.step()
