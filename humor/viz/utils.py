...
def viz_smplx_seq(body, faces, imw=1080, imh=1080, fps=30, contacts=None,
    ...
    if render_body or vtx_list is not None:
        ...
        #faces = c2c(faces)
        body_mesh_seq = [trimesh.Trimesh(vertices=c2c(body.vertices[i]), faces=faces, vertex_colors=vertex_colors, process=False) for i in range(body.vertices.size(0))]

    if render_joints and joints_seq is None:
        start_t = time.time()
        # only body joints
        joints_seq = [c2c(body.joints[i, :22]) for i in range(body.joints.size(0))]
