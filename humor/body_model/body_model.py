...
from body_model.vertex_ids import vertex_ids as VERTEX_IDS
from body_model.vertex_joint_selector import VertexJointSelector
...
class BodyModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_posedirs=True,
                 dtype=torch.float32,
                 use_vtx_selector=False): # if true, returns additional vertices as joints that correspond to OpenPose joints
        ...
        if '.npz' in bm_path:
            ...
            with open(bm_path, 'rb') as pkl_f:
                smpl_dict = pickle.load(pkl_f, encoding='latin1')
        else:
        ...
        if self.use_vtx_selector:
            print('Using vertex selector in body model...')
            vertex_ids = VERTEX_IDS[self.model_type]
            self.vertex_joint_selector = VertexJointSelector(vertex_ids=vertex_ids, use_hands=True, use_feet_keypoints=True)
        ...
        jreg = smpl_dict['J_regressor']
        if not isinstance(jreg, np.ndarray):
            jreg = jreg.todense()
        self.register_buffer('J_regressor', torch.tensor(jreg, dtype=dtype))
