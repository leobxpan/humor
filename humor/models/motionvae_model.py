...
from utils.transforms import convert_to_rotmat, compute_world2aligned_mat, compute_world2aligned_joints_mat, rotation_matrix_to_angle_axis, batch_rodrigues
from body_model.utils import SMPL_JOINTS, SMPLX_PATH
from body_model.body_model import BodyModel
from datasets.amass_discrete_dataset import RETURN_CONFIGS, data_name_list, data_dim
...
def step(model, loss_func, data, dataset, device, cur_epoch, mode='train', use_gt_p=1.0):
    ...
    if not use_sched_samp:
        ...
        sched_samp_out = model.scheduled_sampling(x_past, x_t, init_input_dict, p=use_gt_p, 
                                                                    ...
                                                                    need_global_out=(not model.detach_sched_samp)) # each dict element is B x T x S_out x D
...
class MotionvaeModel(nn.Module):
    ...
    def __init__(self,  in_rot_rep='aa', 
                        ...
                        latent_size=128,
                        steps_in=1,
                        steps_out=1, # each will use it's own decoder
                        out_step_size=1, # num frames between each output step
                        ...
                        detach_sched_samp=True, # if true, the decoder predicts both the mean and variance of observation
        ...
        if model_data_config == 'smpl+verts': # special case where we need joints data to be returned (to calc trans2joint transform) but will not actually use it as input/output
            self.data_names.remove('joints')
            self.aux_in_data_names = ['joints']
        self.pred_contacts = False
        if model_data_config.find('contacts') >= 0: # network is outputting contact classification as wlell and need to supervise, but not given as input to net.
            self.data_names.remove('contacts')
            self.aux_out_data_names = ['contacts']
            self.pred_contacts = True

        self.use_smpl_frame = RETURN_CONFIGS[model_data_config]['use_smpl_frame'] # which coordinate frame to use as input
        self.need_trans2joint = self.use_smpl_frame and ('joints' in self.data_names or 'verts' in self.data_names)
        ...
        # dimension of input feature at each step (trans + root rot + joint rot + joint pos + joint vel)
        # self.input_data_dim = 3 + self.input_rot_dim + self.input_rot_dim*NUM_BODY_JOINTS + NUM_SMPL_JOINTS*3 + NUM_SMPL_JOINTS*3
        self.input_data_dim = sum(self.input_dim_list)

        print(self.data_names)
        print(self.input_dim_list)
        print(self.input_data_dim)
        ...
        if self.posterior_arch == 'mlp':
            ...
            self.encoder = MLP(layers=layer_list, # mu and sigma output
                               ...
                               use_bn=True # actually group norm
                           )
    ...
    def scheduled_sampling(self, x_past, x_t, init_input_dict, p=0.5, gender=None, betas=None, need_global_out=True):
        ...
        if self.use_smpl_frame and self.need_trans2joint:
        ...
        for t in range(T):
            ...
            if self.use_smpl_frame:
                ...
                step0_joints = None
                if self.detach_sched_samp:
                    step0_joints = x_pred_dict['joints'][:,0,:].reshape((B, J, 3)).detach()
                else:
                    step0_joints = x_pred_dict['joints'][:,0,:].reshape((B, J, 3))
                world2aligned_rot = compute_world2aligned_joints_mat(step0_joints)
                world2aligned_trans = torch.cat([-step0_joints[:,0,:2], torch.zeros((B,1)).to(x_past)], axis=1)
            ...
            if need_global_out:
                ...
                if self.use_smpl_frame:
                    ...
                    step0_joints = None
                    if self.detach_sched_samp:
                        step0_joints = cur_world_dict['joints'][:,0,:].reshape((B, J, 3)).detach()
                    else:
                        step0_joints = cur_world_dict['joints'][:,0,:].reshape((B, J, 3))
                    global_world2local_trans = torch.cat([-step0_joints[:,0:1,:2], torch.zeros((B, 1, 1)).to(x_past)], axis=2)
        ...
        if need_global_out:
            ...
            for k in pred_global_seq[0].keys():
                ...
                # print(pred_global_seq_out[k].size())
        ...
        for k in pred_local_seq[0].keys():
            ...
            # print(pred_local_seq_out[k].size())
    ...
    def apply_world2local_trans(self, world2local_trans, world2local_rot, trans2joint, input_dict, output_dict, invert=False):
        ...
        for k, v in input_dict.items():
            ...
            # if k in ['pose_body', 'pose_body_vel', 'joints_orient_vel', 'posterior_distrib', 'prior_distrib']:
    ...
    def roll_out(self, x_past, init_input_dict, num_steps, use_mean=False, 
        ...
        if canonicalize_input:
            ...
            if self.use_smpl_frame:
                ...
                step0_joints = cur_input_dict['joints'][:,-1,:].reshape((B, J, 3))
                world2aligned_rot = compute_world2aligned_joints_mat(step0_joints)
                world2aligned_trans = torch.cat([-step0_joints[:,-1,:2], torch.zeros((B,1)).to(step0_joints)], axis=1)
            # compute trans2joint
            if self.use_smpl_frame and self.need_trans2joint:
    ...
    def infer_global_seq(self, global_seq, full_forward_pass=False):
        ...
        for t in range(T-1):
            ...
            if self.use_smpl_frame:
                ...
                step0_joints = global_seq['joints'][:,t,:].reshape((B, J, 3))
                world2aligned_rot = compute_world2aligned_joints_mat(step0_joints)
                world2aligned_trans = torch.cat([-step0_joints[:,0,:2], torch.zeros((B,1)).to(step0_joints)], axis=1)

            # compute trans2joint at first step
            if t == 0 and self.use_smpl_frame and self.need_trans2joint:
            ...
            for k in global_seq.keys():
                ...
                # cur_data_dict[k] = global_seq[k][:,t:t+2,:] # NOTE: assumes one input frame, one output frame
...
class MLP(nn.Module):
    def __init__(self, layers=[3, 128, 128, 3], nonlinearity=nn.ReLU, use_bn=True, skip_input_idx=None):
        '''
