...
def eval_sampling(model, test_dataset, test_loader, device,
    ...
    from body_model.body_model import BodyModel
    from body_model.utils import SMPLH_PATH, SMPLX_PATH
...
def viz_eval_samp(x_pred_dict, meta, neutral_bm,
    ...
    betas = meta['betas'].to(x_pred_dict[list(x_pred_dict.keys())[0]].device)
    human_verts_list = []
    human_faces = None
    for b in range(B):
        bm_world = neutral_bm
        ...
        body_pred = bm_world(pose_body=pred_world_pose_body[b], 
                        pose_hand=None,
                        betas=torch.zeros(T, 16).to(pred_world_pose_body),
