...
from body_model.utils import SMPLH_PATH, SMPL_JOINTS, VPOSER_PATH, SMPL_PARENTS, MOJO_VERTS
...
class MotionvaeLoss(nn.Module):
    ...
    def __init__(self,
                    ...
                    smpl_batch_size=160):
        super(MotionvaeLoss, self).__init__()
        '''
    ...
    def forward(self, pred_dict, gt_dict, cur_epoch, gender=None, betas=None):
        ...
        for cur_key in gt_dict.keys():
            ...
            if cur_regr_weight > 0.0:
                ...
                if cur_key == 'root_orient' or cur_key == 'pose_body':
                    ...
                    # print(torch.max(self.regr_loss(pred_val, gt_val)))
        ...
        if self.use_smpl_losses:
            ...
            for gender_name in gender_names:
                ...
                # viz_smpl_seq(pred_body, imw=1080, imh=1080, fps=30, contacts=None,
                #         render_body=True, render_joints=True, render_skeleton=False, render_ground=True,
                #         joints_seq=pred_dict['joints'][gender_idx].reshape((B, -1, 3)),
                #         joints_vel=pred_dict['joints_vel'][gender_idx].reshape((B, -1, 3)))
                # viz_smpl_seq(gt_body, imw=1080, imh=1080, fps=30, contacts=None,
                #         render_body=True, render_joints=True, render_skeleton=False, render_ground=True,
                #         joints_seq=gt_dict['joints'][gender_idx].reshape((B, -1, 3)),
                #         joints_vel=gt_dict['joints_vel'][gender_idx].reshape((B, -1, 3)))
            ...
            pred_verts = pred_mesh[:,MOJO_VERTS,:]
            gt_verts = gt_mesh[:,MOJO_VERTS,:]
            ...
            if self.smpl_joint_loss_weight > 0.0:
                ...
                # print('smpl_joint_loss')
                # print(smpl_joint_loss.size())
            ...
            if self.smpl_mesh_loss_weight > 0.0:
                ...
                # print('smpl_mesh_loss')
                # print(smpl_mesh_loss.size())
            ...
            if self.smpl_joint_consistency_loss_weight > 0.0:
                ...
                # print('smpl_joint_consistency_loss')
                # print(smpl_joint_consistency_loss.size())
            ...
            if self.smpl_vert_consistency_loss_weight > 0.0:
                if not 'verts' in pred_dict.keys():
                    ...
                    exit()
                regressed_verts = pred_dict['verts'].reshape((B, len(MOJO_VERTS), -1))
                ...
                # print('smpl_vert_consistency_loss')
                # print(smpl_vert_consistency_loss.size())
