...
class GIMOFitDataset(Dataset):
    ...
    def __init__(self, data_path,
        ...
        if self.return_points:
            ...
            self.male_bm = BodyModel(bm_path=male_bm_path, num_betas=10, batch_size=self.seq_len).to(self.device)
            self.female_bm = BodyModel(bm_path=female_bm_path, num_betas=10, batch_size=self.seq_len).to(self.device)

    def __len__(self):
        return self.gimo_dataset.__len__()
