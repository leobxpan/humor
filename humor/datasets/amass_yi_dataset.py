...
class AmassYiDataset(Dataset):
    ...
    def __init__(self, split='train', 
        ...
        '''
        super(AmassYiDataset, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        # smpl root translation as residuals except z-axis
        trans_in = trans_out = None
        if self.return_cfg['trans']:
            trans_data = np.zeros((self.sample_num_frames, slice_size, 3))
            res_trans = np.zeros((self.sample_num_frames - 1, slice_size, 3))
            for t in range(self.sample_num_frames):
